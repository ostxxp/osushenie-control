...
const actionNames: Record<string, string> = { assigned: 'Назначение', started: 'Начало работы', submitted: 'Отправка на проверку', accepted: 'Принятие', rejected: 'Возврат', branch_selected: 'Выбор ветки', branch_cleared: 'Сброс ветки', attachment_uploaded: 'Загрузка файла', attachment_deleted: 'Удаление файла' }
...
export default function ActivityPage() {
  ...
  return <div className="space-y-5"><div className="rounded-3xl bg-white p-5 shadow-sm"><h1 className="text-3xl font-semibold">История действий</h1><div className="mt-4 flex flex-wrap gap-3"><select className="select select-bordered" value={objectId} onChange={(e) => setObjectId(e.target.value)}><option value="">Все объекты</option>{objects.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</select><select className="select select-bordered" value={action} onChange={(e) => setAction(e.target.value)}><option value="">Все действия</option>{Object.entries(actionNames).map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></div></div>
    <div className="space-y-2">{items.map((item) => <article key={item.id} className="rounded-2xl border bg-white p-4"><div className="flex flex-wrap justify-between gap-2"><strong>{item.task_title}</strong><time className="text-sm text-slate-500">{formatDateTimeRu(item.created_at)}</time></div><div className="mt-1 text-sm text-slate-600">{item.actor_full_name || 'Система'} · {actionNames[item.action] || item.action} · {item.object_name}</div></article>)}</div>
