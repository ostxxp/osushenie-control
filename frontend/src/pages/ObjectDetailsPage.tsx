        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_220px] lg:items-stretch">
          <div className="min-w-0">
            ...
            <div className="mb-5 min-w-0">
              {isEditing ? (
                <input
                  className="input min-h-0 w-full rounded-xl border-slate-200 px-3 py-2 text-2xl font-semibold text-slate-950 focus:border-[#ff4539] focus:outline-none"
                  value={editForm.name}
                  onChange={(e) => updateEditForm('name', e.target.value)}
                  aria-label="Название объекта"
                />
              ) : (
                <h1 className="inline break-words text-2xl font-semibold leading-tight text-slate-950 sm:text-3xl">
                  {objectItem.name}
                </h1>
              )}

              {!isEditing && !objectItem.is_active && (
                <span className="badge ml-2 h-auto max-w-full border border-slate-200 bg-slate-100 px-3 py-1 text-center text-slate-600">
                  Объект неактивен
                </span>
              )}

              {canEditObject && !isEditing && (
                <div ref={actionsMenuRef} className="relative ml-2 inline-block align-middle">
                  <button
                    type="button"
            ...
            {isEditing ? (
              ...
              <div className="grid grid-cols-1 gap-x-14 gap-y-4 text-sm sm:grid-cols-2 xl:max-w-3xl">
                <div>
                  ...
                  <div className="mt-1 break-words font-semibold text-slate-950">{objectItem.address}</div>
                </div>
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Тип объекта</div>
                  <div className="mt-1 break-words font-semibold text-slate-950">{objectType || '—'}</div>
                </div>
                <div>
